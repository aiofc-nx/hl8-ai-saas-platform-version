...
**特性分支**：`001-domain-core`  
**创建日期**：2025-11-11  
**状态**：草稿  
**输入**：用户描述：“根据docs/designs/platform-domain-baseline.md和docs/designs/platform-domain-design.md开发@hl8/domain-core，我已经创建了项目的项目结构”
...
**独立验证方式**：通过在示例领域模块中引用 `@hl8/domain-core` 并创建聚合，验证租户断言、审计字段和软删除状态均可直接使用。
...
- **FR-009**：系统必须提供模板或示例文档，指导领域团队如何在新模块中接入 `@hl8/domain-core` 并满足 DDD + 多租户要求。
...
- 应用层与基础设施层遵循现有 CQRS/ES/EDA 基线，可直接消费 `@hl8/domain-core` 提供的领域事件与接口。
...
- **SC-001**：平台内新增领域模块中至少 90% 在首个迭代内完成 `@hl8/domain-core` 接入并通过审计、租户隔离验证。
- **SC-002**：基于 `@hl8/domain-core` 创建聚合的脚手架或示例项目，可在 30 分钟内完成从初始化到触发首个领域事件的演示。
