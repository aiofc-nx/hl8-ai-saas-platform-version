/**
 * @fileoverview 激活租户命令处理器
 * @description 处理激活租户命令，包括状态校验、聚合更新、事件保存和发布
 *
 * ## 业务规则
 *
 * ### 权限校验
 ...
 * ### 业务校验
 * - 租户必须存在
 * - 租户状态必须允许激活（Initialized 或 Suspended）
 * - 已归档的租户不能激活
 *
 * ### 执行流程
 * 1. 从仓储加载租户聚合
 ...
 * 激活租户命令处理器
 *
 * @description 处理激活租户命令
 */
@CommandHandler(ActivateTenantCommand)
export class ActivateTenantHandler extends CaslCommandHandler<
...
> {
   ...
   */
  protected async handle(command: ActivateTenantCommand): Promise<void> {
    // 1. 从仓储加载租户聚合
    ...
    if (!tenant.status.isInitialized() && !tenant.status.isSuspended()) {
      throw new DomainException(
        `当前状态 ${tenant.status.value} 不能激活，只能从 Initialized 或 Suspended 状态激活`,
      );
    }
